...
@pytest.mark.parametrize("mesh_cls", [TriangleMesh, QuadMesh, HexMesh])
def test_no_edges_zero(mesh_cls):
    mesh = mesh_cls(weights=MeshWeights())
    G = nx.Graph()
    G.add_nodes_from(range(20))
    assert mesh.score(G) == pytest.approx(0.0), "No edges must score 0.0"
...
def test_disconnected_is_penalized(mesh_cls):
    ...
    assert s < mesh.weights.cycle_floor - 1e-9
    assert s <= mesh.weights.forest_ceiling + 1e-9
...
def test_cycle_beats_forest(mesh_cls):
    ...
    assert s_forest <= mesh.weights.forest_ceiling + 1e-9
    assert s_cycle  >= mesh.weights.cycle_floor - 1e-9
