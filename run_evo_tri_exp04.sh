...
python -m guca.cli.run_ga_hydra --config-path=../../../configs/trimesh_phase01 --config-name=exp004 \
  experiment.name="exp004.01_tregen5K" \
  ga.generations=5000 ga.pop_size=200 \
  ga.checkpoint.save_every=1000

python -m guca.cli.run_ga_hydra --config-path=../../../configs/trimesh_phase01 --config-name=exp004 \
  experiment.name="exp004.02_tregen10K" \
  ga.generations=10000 ga.pop_size=200 \
  ga.checkpoint.save_every=1000

python -m guca.cli.run_ga_hydra --config-path=../../../configs/trimesh_phase01 --config-name=exp004 \
  experiment.name="exp004.03_tregen10K" \
  ga.generations=10000 ga.pop_size=200 \
  ga.checkpoint.save_every=1000
